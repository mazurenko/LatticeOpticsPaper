...
To prevent external influences from contaminating large data sets, we have implemented an independent monitoring system to verify appropriate pointing of both lattice laser beams.
...
While the position of the atoms is dictated by the potential generated from the ingoing and retro-reflected laser beams and the monitoring camera only observes the position of the ingoing beam.
In order to verify that the monitor camera \dots
purposeful misalignment of the ingoing beam shows a clear correlation between the monitor camera signal and the 

Typical operation of the experiment involves repeated realizations of the ultracold atomic system, with a cycle time of 10s of seconds, with a lattice duty cycle of about 10\%.
