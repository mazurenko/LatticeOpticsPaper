...
Using real-time monitoring of the resulting optical lattice, we find the stability of the lattice site positions to be well below the lattice spacing over the course of hours. The pointing stability of the optical lattice beams is around one lattice spacing and the long-term (six month) relative stability of the lattice spacing itself is 0.5\% RMS.
\end{abstract}
\maketitle
...
In practical terms, this requires approximately \unit{20}{W} per lattice axis for a beam waist of around $80 \mu\mathrm{m}$ and the four-fold enhancement in optical depth provided by the reflection from the superpolished substrate~\cite{Huber2014}. We achieve such high laser powers using Yb-doped fiber amplifiers (Nufern NuAmp SUB1174-34).
...
Worse yet, the digital control system is suspected of causing ground loops that add noise and interference to the analog systems.
...
\begin{enumerate}
  \item Its effect on the RIN can limit the system temperature and set heating rate. This can limit the amount of time available for atom interaction~\cite{Blatt2015}. 
  \item It needs to avoid varying the the beam pointing because the trap position matters during this phase of the experiment.
...
The PLL is locked to the \unit{10}{MHz} frequency reference distributed around the lab to ensure absolute frequency stability.
...
This device allows us to drive the AOM with a single, clean tone.
The spectrum and relative intensity noise of the PLL can be found in Fig.~\ref{fig:pll}.
Vitally, the spectrum of the PLL exhibits phase noise comparable to that of a typical VCO but without the slow frequency drift characteristic to these devices.
...
We use a simple, primarily integrating design in three stages, as seen in Fig.~\ref{fig:circuits}(c), based on a low-noise op-amp (Texas Instruments OPA211).
...
In this regime, the integrator accumulates a large positive or negative voltage, such that when the loop is mended, the system appears to briefly rail before control is restored.
...
This is done by optimizing the on-site trap frequency of lattice, measured using lattice-modulation spectroscopy.
