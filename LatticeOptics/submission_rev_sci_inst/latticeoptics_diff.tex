...
%DIF ADD latticeoptics.tex                            Sat Oct 13 16:08:45 2018
...
Using real-time monitoring of the resulting optical lattice, we find the stability of the lattice site positions to be well below the lattice spacing \DIFdelbegin \DIFdel{for several }\DIFdelend \DIFaddbegin \DIFadd{over the course of }\DIFaddend hours. The pointing stability of the optical lattice beams is around one lattice spacing and the long-term (six month) relative stability of the lattice spacing itself is 0.5\% RMS.
\end{abstract}
\maketitle
...
These amplifiers generate \DIFdelbegin \DIFdel{enough }\DIFdelend \DIFaddbegin \DIFadd{sufficient }\DIFaddend light to perform site resolved imaging~\cite{Parsons2015}, but also substantially increase the level of intensity and pointing noise.
...
We expect, however, that both the specific engineering choices and the general design practices detailed in this work will be widely applicable to experimentalists working with performant optical lattices used for a variety of purposes.
}\DIFaddend 
...
Worse yet, \DIFdelbegin \DIFdel{it }\DIFdelend \DIFaddbegin \DIFadd{the digital control system }\DIFaddend is suspected of causing ground loops \DIFdelbegin \DIFdel{which }\DIFdelend \DIFaddbegin \DIFadd{that }\DIFaddend add noise and interference to the analog systems.
...
In contrast, the low-power loop that controls the lattice laser during the \emph{interaction} phase of the experiment is critically important:
 \begin{enumerate} 
  \item Its effect on the RIN can limit the system temperature \DIFaddbegin \DIFadd{and set heating rate. This can limit the amount of time available for atom interaction~\mbox{%DIFAUXCMD
\cite{Blatt2015}}%DIFAUXCMD
}\DIFaddend . 
  \item It \DIFdelbegin \DIFdel{can affect the pointing}\DIFdelend \DIFaddbegin \DIFadd{needs to avoid varying the the beam pointing because the trap position matters during this phase of the experiment}\DIFaddend .
...
\DIFdelbegin \DIFdel{To ensure frequency stability (which translates directly into beam pointing of the laser), the }\DIFdelend %DIF > To ensure frequency stability (which translates directly into beam pointing of the laser), the PLL is locked to the \unit{10}{MHz} frequency reference distributed around the lab.
\DIFaddbegin \DIFadd{The }\DIFaddend PLL is locked to the \unit{10}{MHz} frequency reference distributed around the lab \DIFaddbegin \DIFadd{to ensure absolute frequency stability}\DIFaddend .
...
Vitally, the spectrum of the PLL exhibits phase noise comparable to that of a typical VCO \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{but }\DIFaddend without the slow frequency drift characteristic to these devices.
...
In this regime, the integrator accumulates a large positive or negative voltage, such that when \DIFdelbegin \DIFdel{control is restored}\DIFdelend \DIFaddbegin \DIFadd{the loop is mended}\DIFaddend , the system appears to briefly rail before control is restored.
...
This is done by optimizing the on-site trap frequency of lattice, measured using \DIFdelbegin \DIFdel{parametric heating}\DIFdelend \DIFaddbegin \DIFadd{lattice-modulation spectroscopy}\DIFaddend .
