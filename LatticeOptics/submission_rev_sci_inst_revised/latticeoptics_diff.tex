...
%DIF ADD latticeoptics.tex                            Sat Dec 29 15:17:48 2018
%DIF < \documentclass[twocolumn,aps,pra,preprintnumbers,bibnotes]{revtex4-1}
%DIF -------
\documentclass[twocolumn,aip,rsi,reprint,bibnotes]{revtex4-1} %DIF > 
%DIF -------
...
Using real-time monitoring of the resulting optical lattice, we find the stability of the lattice site positions to be well below the lattice spacing \DIFdelbegin \DIFdel{for several }\DIFdelend \DIFaddbegin \DIFadd{over the course of }\DIFaddend hours. The \DIFdelbegin \DIFdel{pointing }\DIFdelend stability of the \DIFaddbegin \DIFadd{underlying harmonic trap produced by the }\DIFaddend optical lattice beams is around one lattice spacing and the long-term (six month) relative \DIFaddbegin \DIFadd{RMS }\DIFaddend stability of the lattice spacing itself is 0.5\%\DIFdelbegin \DIFdel{RMS}\DIFdelend .
\end{abstract}
\maketitle
...
These amplifiers generate \DIFdelbegin \DIFdel{enough }\DIFdelend \DIFaddbegin \DIFadd{sufficient }\DIFaddend light to perform \DIFdelbegin \DIFdel{site resolved }\DIFdelend \DIFaddbegin \DIFadd{site-resolved }\DIFaddend imaging~\cite{Parsons2015}, but also substantially increase the level of intensity and pointing noise.
...
The optical system is engineered to be passively stable in terms of pointing, but the position and phase of the optical lattice are monitored in real time \DIFdelbegin \DIFdel{, }\DIFdelend and beam pointing can be controlled with remotely actuated mirrors.
...
The lattice laser system is seeded by an Innolight Mephisto laser \DIFdelbegin \DIFdel{, which provides }\DIFdelend \DIFaddbegin \DIFadd{providing }\DIFaddend approximately \unit{2}{W} of \unit{1064}{nm} light.
...
\begin{figure*}
  \begin{center}
    ...
    \caption{\textbf{Laser noise of the lattice system.} (a) Shows the laser noise of unmodified and free-running Nufern fiber amplifiers and the Mephisto seed, as well as an independent Mephisto MOPA 55W (not used for the optical lattice presented in this manuscript). (b) Shows the open-loop and closed-loop behavior of the high-power feedback loop using the  modified fiber amplifiers, as described in the main text. (c) Shows the open-loop behavior of the low-power feedback loop using the modified fiber amplifiers \DIFdelbeginFL \DIFdelFL{, including an additional low-pass filtering stage to limit the loop bandwidth}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{at various constant powers}\DIFaddendFL . (d) Closed-loop behavior of the low-power feedback loop \DIFaddbeginFL \DIFaddFL{(}\DIFaddendFL without additional low-pass filtering\DIFaddbeginFL \DIFaddFL{)}\DIFaddendFL , measured on the same setup as panel (c).}\label{fig:noises}
  \end{center}
\end{figure*}

\subsection{Fiber amplifier system}
Quantum gas microscopy of \DIFdelbegin \DIFdel{Li}\DIFdelend $^6$\DIFdelbegin \DIFdel{requires single site }\DIFdelend \DIFaddbegin \DIFadd{Li requires single-site }\DIFaddend trap frequencies of approximately \unit{1}{MHz} \DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\cite{Parsons2015} }%DIFAUXCMD
}\DIFdelend to be in the Lamb-Dicke regime necessary for Raman sideband cooling\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{Parsons2015}}%DIFAUXCMD
}\DIFaddend .

Such trap frequencies may only be attained using tightly focused, high-power beams.
In practical terms, this requires approximately \unit{20}{W} per lattice axis for a beam waist of around \DIFdelbegin \DIFdel{$80 \mu\mathrm{m}$ }\DIFdelend \DIFaddbegin \unit{80}{\mu m} \DIFaddend and the four-fold enhancement in \DIFdelbegin \DIFdel{optical }\DIFdelend \DIFaddbegin \DIFadd{lattice }\DIFaddend depth provided by the reflection from the superpolished substrate\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{Huber2014}}%DIFAUXCMD
...
Worse yet, \DIFdelbegin \DIFdel{it is suspected of causing ground loops which }\DIFdelend \DIFaddbegin \DIFadd{the digital control system causes ground loops that }\DIFaddend add noise and interference to the analog systems.
...
Despite the presence of a water-cooled cold-plate for the gain fiber and pump diodes, the high power supply is \DIFdelbegin \DIFdel{air cooled }\DIFdelend \DIFaddbegin \DIFadd{air-cooled }\DIFaddend by an internal fan with variable frequency.
This fan adds noise at acoustic frequencies and degrades pointing stability by increasing \DIFdelbegin \DIFdel{vibrations on }\DIFdelend \DIFaddbegin \DIFadd{vibration of }\DIFaddend the output fiber tip.
...
Experimentally, we have found that more power can be gained by running the amplifiers at even hotter temperatures\DIFaddbegin \DIFadd{, possibly at the expense of a shortened lifetime}\DIFaddend .
...
Due to the high laser power, fused silica glass and \DIFdelbegin \DIFdel{IBS }\DIFdelend \DIFaddbegin \DIFadd{ion-beam-sputtered (IBS) }\DIFaddend coatings are used wherever possible.
...
The optical isolators are based on \unit{5}{mm} aperture Tb$_3$Ga$_5$O\DIFdelbegin \DIFdel{$_12$ }\DIFdelend \DIFaddbegin \DIFadd{$_{12}$ }\DIFaddend (TGG) isolators (Thorlabs IO-5-1064-VHP), where we replaced the default polarizers (BK7 Brewster plates) by IBS-coated fused silica plate polarizers.
...
After the beam has been cleaned up and isolated, it is important to consider two aspects of the desired experiment:
 \begin{enumerate} 
\item The lattice power must be continuously tunable from around \unit{10}{\mu{}W} to around \unit{20}{W}, corresponding to lattice depths in the approximate range of \DIFdelbegin \DIFdel{$10^-3$ }\DIFdelend \DIFaddbegin \DIFadd{$10^{-3}$ }\DIFaddend to $2 \times 10^3 E_R$, where $E_R$ is the geometric recoil energy corresponding to the lattice spacing~\cite{Blatt2015}.
...
\DIFdelbegin \DIFdel{Fortunately, the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend spatial phase of the optical lattice is set by the retro-reflector mirror, which does not move, and is not affected by the rotation of the Berek compensator.
...
A polarization rotation \DIFdelbegin \DIFdel{by }\DIFdelend \DIFaddbegin \DIFadd{of }\DIFaddend $2 \pi$ is produced by approximately $3^\circ$ of axial rotation of the quartz plate.
...
To accomplish this, the low-power loop is always manually railed to its high extreme when the high-power loop is active\DIFdelbegin \DIFdel{, and }\DIFdelend \DIFaddbegin \DIFadd{.
...
Conversely, }\DIFaddend the digital high-power loop is disabled \DIFaddbegin \DIFadd{in software }\DIFaddend when the low-power loop is active.
...
Even if the loop was \DIFdelbegin \DIFdel{exceedingly }\DIFdelend noisy, the heating would be entirely compensated by the simultaneous Raman cooling~\cite{Parsons2015}.
...
In contrast, the low-power loop that controls the lattice laser during the \emph{interaction} phase of the experiment is critically important:
 \begin{enumerate} 
  \item Its effect on the RIN can limit the system temperature \DIFaddbegin \DIFadd{and dominate heating rates, limiting the amount of time available for atom interaction~\mbox{%DIFAUXCMD
\cite{Blatt2015}}%DIFAUXCMD
}\DIFaddend .
  \item It \DIFdelbegin \DIFdel{can affect the pointing}\DIFdelend \DIFaddbegin \DIFadd{needs to avoid varying the beam pointing because the trap position matters during this phase of the experiment}\DIFaddend .
  \item It must have closed-loop bandwidths \DIFdelbegin \DIFdel{of }\DIFdelend $\geq\unit{1}{kHz}$, and open-loop modulation bandwidths $\geq \unit{100}{kHz}$.
 \end{enumerate} 
To this end, we use an \DIFdelbegin \DIFdel{all-custom }\DIFdelend \DIFaddbegin \DIFadd{nonlinear feedback }\DIFaddend loop based on a TeO$_2$ AOM (Gooch \& Housego AOMO 3080-198), where the power of the RF supplied to the AOM is used to actuate the fast, low-power feedback loop.
...
\DIFdelbegin \DIFdel{To prevent damage to the low-power photodiode when the system is at its highest powers, a shutter (Stanford Research Systems SR475) blocks the light when the high-power loop is in use.
}\DIFdelend %DIF > To prevent damage to the low-power photodiode when the system is at its highest powers, a shutter (Stanford Research Systems SR475) blocks the light when the high-power loop is in use.
...
\DIFdelbegin \DIFdel{To ensure frequency stability (which translates directly into beam pointing of the laser), the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend PLL is locked to the \unit{10}{MHz} frequency reference distributed \DIFdelbegin \DIFdel{around the lab }\DIFdelend \DIFaddbegin \DIFadd{throughout the lab to ensure absolute frequency stability}\DIFaddend .
In addition to PLL functionality, the board also contains two digitally-controlled, high-isolation RF switches, giving a total \DIFaddbegin \DIFadd{RF }\DIFaddend isolation exceeding \unit{90}{dB}.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{We need to maintain the relatively low phase-noise characteristics of the VCO to prevent phase- to amplitude-noise conversion in the AOM , which partially downconverts the VCO phase noise to RIN at acoustic frequencies.
The spectrum and relative intensity noise of the PLL can be found in Fig.~\ref{fig:pll}.
Vitally}\DIFdelend \DIFaddbegin \DIFadd{This device allows us to drive the AOM with a single, clean tone ($<$}\unit{90}{dBc/Hz} \DIFadd{at }\unit{200}{kHz} \DIFadd{away from the carrier).
%DIF > The spectrum and relative intensity noise of the PLL can be found in Fig.~\ref{fig:pll}.
Thus}\DIFaddend , the spectrum of the PLL exhibits phase noise comparable to that of a typical VCO \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{but }\DIFaddend without the slow frequency drift characteristic to these devices.
Elimination of the carrier drift is worth the added phase noise \DIFdelbegin \DIFdel{, however, }\DIFdelend because any drift of the frequency affects the pointing of the laser beam passing through the AOM, which would misalign the optical lattice.

\DIFdelbegin %DIFDELCMD < \begin{figure}
%DIFDELCMD <   \begin{center}
%DIFDELCMD <     \includegraphics{Figure7.pdf}
%DIFDELCMD <     %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\textbf{\DIFdelFL{PLL performance}} %DIFAUXCMD
\DIFdelFL{(a) The spectrum of the PLL, exhibiting phase noise of better than $-90$ dBc/Hz at $100$~kHz away from the carrier. (b) The RIN of the local oscillator based on the PLL. The noise level is seen to be at or below the noise floor of the measurement device.}}%DIFAUXCMD
%DIFDELCMD < %DIFDELCMD < \label{fig:pll}%%%
%DIFDELCMD <   \end{center}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend %DIF > \begin{figure}
%DIF >   \begin{center}
%DIF >     \includegraphics{Figure7.pdf}
%DIF >     \caption{\textbf{PLL performance} (a) The spectrum of the PLL, exhibiting phase noise of better than $-90$ dBc/Hz at $100$~kHz away from the carrier. (b) The RIN of the local oscillator based on the PLL. The noise level is seen to be at or below the noise floor of the measurement device.}\label{fig:pll}
%DIF >   \end{center}
%DIF > \end{figure}
...
The chosen photodiode (Hamamatsu G8370-01) is an \DIFdelbegin \DIFdel{InGaS }\DIFdelend \DIFaddbegin \DIFadd{InGaAs }\DIFaddend based, small-area, low-noise design with a high efficiency and low thermal sensitivity at \unit{1064}{nm}.
...
We use a simple \DIFdelbegin \DIFdel{, purely integrating design based }\DIFdelend \DIFaddbegin \DIFadd{integrating design }\DIFaddend in three stages, as seen in Fig.~\ref{fig:circuits}(c), based on a low-noise op-amp (Texas Instruments OPA211).
...
In effect, if the rest of the feedback system was linear, this would correspond to a quadratically increasing gain until the AOM is saturated, which \DIFdelbegin \DIFdel{could lead }\DIFdelend \DIFaddbegin \DIFadd{leads }\DIFaddend to slow behavior at low signal levels and ringing at higher signal levels.
...
\DIFaddbegin \DIFadd{The loop transfer function is sensitive to the specific tuning parameters of the nonlinear controller resulting in slightly different transfer functions for the two axes, although both exhibit approximately constant behavior over the desired range of setpoints.
}\DIFaddend Another function of the last stage is to clamp the voltage to be strictly positive by including a diode clamp on the output to prevent unstable positive feedback behavior~\cite{Horowitz2015}.
...
In this regime, the integrator accumulates a large positive or negative voltage, such that when \DIFdelbegin \DIFdel{control is restored}\DIFdelend \DIFaddbegin \DIFadd{the loop is mended}\DIFaddend , the system \DIFdelbegin \DIFdel{appears to briefly rail }\DIFdelend \DIFaddbegin \DIFadd{briefly rails }\DIFaddend before control is restored.
...
Thus, in addition to the switch, when the system is inactive, the set-point is set to the dark signal from the photodiode ($\approx$\unit{200}{\mu{}V}\DIFaddbegin \DIFadd{, calibrated specifically to each photodiode}\DIFaddend ), making the error signal very small.
...
However, a small amount of tuning can increase the bandwidth up to $\approx$\unit{150}{kHz}, which is limited by the propagation delay of the \DIFaddbegin \DIFadd{acoustic wave in the }\DIFaddend AOM.
...
\DIFdelbegin \DIFdel{Although noise }\DIFdelend \DIFaddbegin \DIFadd{Noise }\DIFaddend near DC is lowered (as expected from a closed-loop system), noise outside the loop bandwidth is increased in the range of 10 to \unit{450}{kHz} as seen in subfigures (c) and (d).
This can be explained by the Johnson noise~\cite{Horowitz2015} on the \unit{1}{k\Omega} \DIFaddbegin \DIFadd{op-amp }\DIFaddend input resistors, amplified by the amplification stages of the loop filter.
...
For this reason, we use a combination of quarter waveplate (QWP) and half waveplate (HWP) to optimize the interference contrast of each lattice \DIFaddbegin \DIFadd{axis}\DIFaddend .
This is done by optimizing the on-site trap frequency of lattice, measured using \DIFdelbegin \DIFdel{parametric heating}\DIFdelend \DIFaddbegin \DIFadd{lattice-modulation spectroscopy~\mbox{%DIFAUXCMD
\cite{Blatt2015}}%DIFAUXCMD
}\DIFaddend .
...
The position of the atomic cloud, however, is not determined by the phase difference of the retro-reflecting lattice \DIFdelbegin \DIFdel{beam}\DIFdelend \DIFaddbegin \DIFadd{beams}\DIFaddend , but by the underlying harmonic confinement \DIFaddbegin \DIFadd{generated by their transverse Gaussian profiles}\DIFaddend .
