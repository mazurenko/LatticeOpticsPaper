...
In this work we discuss a high power optical lattice system that exhibits levels of intensity noise below $-120\,$dBc/Hz between $1\,$kHz and $3\,$MHz Fourier frequency.
...
The coarse architecture of the system is shown in Fig.~\ref{fig:high_level} and is based on a single low-power, low-noise seed laser that supplies light to an array of four high-power fiber amplifiers (one for a transport beam~\cite{Huber2014}, and one each for the $X,Y,Z$ lattices~\cite{Parsons2016}).
...
This can be as seen by comparing Fig.~\ref{fig:noises} (a) and (b), particularly in the region of approximately $10$ kHz.
...
The rise of the noise floor as a function of the pump diode current can be seen in Fig.~\ref{fig:noises}(a).
...
In order to address these challenges, we use a carefully engineered the optical system shown in Fig.~\ref{fig:optical_layout}.
...
After the isolation stage, we use the sequence of optical elements shown in Fig.~\ref{fig:optical_layout}, consisting of a polarizer, half-wave plate (HWP), Berek compensator mounted on a precision galvo (Thorlabs GVS002, or Camtech 8320K, depending on beam path) half-wave plate, and polarizer.
